...
def RESNET(mode="TrR", blocks=12, weights=None, trainable=False,
  ...
  ## OUTPUT ##
  p_theta   = Dense(25, activation="softmax", **ex)(A)
  p_phi     = Dense(13, activation="softmax", **ex)(A)
  A_sym     = Lambda(lambda x: (x + tf.transpose(x,[0,2,1,3]))/2)(A)
  p_dist    = Dense(37, activation="softmax", **ex)(A)
  p_omega   = Dense(25, activation="softmax", **ex)(A)
  outs      = [p_theta, p_phi, p_dist, p_omega]
  if concat: outs = Concatenate()(outs)  
  ## MODEL ##
...
################################################################################################
# Sergey's TrMRF model
################################################################################################
def TrMRF(blocks=5, DB_DIR="."):
  def seqsep(x):
    L = tf.shape(x)[1]
  ...
  class get_W(Layer):
    ...
    def call(self, inputs):
      ...
      # get two-body; symm; center; remove diagonal
      W = 0.5 * (W + tf.transpose(W,[0,2,1,4,3]))
      W = W - tf.reduce_mean(W,(3,4),keepdims=True)
      W = W * (1-tf.eye(self.L,dtype=tf.float32))[None,:,:,None,None]
  ...
  W,V = get_W()(A)
  model = Model(I,[W,V])
  model.set_weights(np.load(f"{DB_DIR}/model_TrMRF_seqid_retrain_5blocks.npy", allow_pickle=True))
  return model
...
class mk_design_model:
  ...
  def __init__(self, add_pdb=False, add_bkg=False, add_seq=False,
               ...
               add_TrMRF=False, TrMRF_use_O_feat=False,
               add_TrROS=True, n_models=5, specific_models=None, serial=False, diag=0.4,
               pssm_design=False, msa_design=False, feat_drop=0, eps=1e-8,
               DB_DIR=".", lid=[0.3,18.0], uid=[1,0]):

    self.serial = serial
    self.feat_drop = feat_drop
    self.add_TrROS = add_TrROS
    ...
    seqid           = add_input([],"seqid",tf.float32)
    ...
    if add_TrROS:
      ...
      for token in tokens[specific_models]:
        ...
        if self.serial:
          ...
          self.models.append(RESNET(weights=weights, mode="TrR", concat=False)(I_feat))
          
      # average output of models
      if self.serial:
        O_feat = RESNET(mode="TrR", concat=False)(I_feat)
        O_f = {k:O_feat[n] for n,k in enumerate(["theta","phi","dist","omega"])}
        O_feat = Concatenate()(O_feat)
      else:
        O_f = {k:[] for k in ["theta","phi","dist","omega"]}
        O_feat = []
        for model in self.models:
          for n,k in enumerate(["theta","phi","dist","omega"]):
            O_f[k].append(model[n])
          O_feat.append(Concatenate()(model))
        for k in ["theta","phi","dist","omega"]:
          O_f[k] = tf.reduce_mean(O_f[k],0)
        O_feat = tf.reduce_mean(O_feat,0)
    ...
    # TrMRF
    if add_TrMRF:
      print(f"loading model: TrMRF")
      self.TrMRF_model = TrMRF(DB_DIR=DB_DIR)
      if add_TrROS and (add_pdb == False or TrMRF_use_O_feat):
      	def one_hot(x_soft,axis=-1):
      		x_id = tf.math.argmax(x_soft,axis)
      		x_hard = tf.one_hot(x_id,tf.shape(x_soft)[axis])
      		return tf.stop_gradient(x_hard) # - x_soft) + x_soft
      	pdb_out = Concatenate()([one_hot(O_f[k]) for k in ["theta","phi","dist","omega"]])
      else:
        pdb_out = pdb
        
      pdb_seqid = tf.pad(pdb_out,[[0,0],[0,0],[0,0],[0,1]],constant_values=seqid)
      W,V = self.TrMRF_model(pdb_seqid)
      
      I_pred_logits = tf.einsum("xnia,xijab->xnjb",tf.stop_gradient(I_hard),W) 
      I_pred_logits = I_pred_logits + V[:,None,:,:]
      I_pred = tf.nn.softmax(I_pred_logits, -1)
      TrMRF_loss = -K.mean(K.sum(I_hard * K.log(I_pred+eps),-1),[-1,-2])
      add_loss(TrMRF_loss,"TrMRF")
    
    # cross-entropy loss for fixed backbone design
    if add_TrROS and add_pdb:
    ...
    if add_TrROS: add_output(O_feat,"feat")
  ...
  def design(self, inputs,
             ...
             return_traj=False, shuf=True, recompute_loss=False, seqid=1.0):
    ...
    for k in range(opt_iter):   
      ...
      if recompute_loss:
        q = self.predict(inputs, loss_weights=loss_weights, seqid=seqid)
        loss = q["loss"]
    ...
    # recompute output
    p     = self.predict(inputs, loss_weights=loss_weights, seqid=seqid)
    loss_ = to_dict(self.loss_label, p["loss"][0])

    if verbose: print("FINAL loss:"+str(loss_).replace(' ',''))
    output = {"loss":loss_, "I":inputs["I"], "losses":losses, "traj":traj}
    if self.add_TrROS: output["feat"] = p["feat"][0]
    return output

  ###############################################################################
  def predict(self, inputs, loss_weights=None, loss2_weights=None,
              sample=False, hard=True, temp=1.0, train=False, seqid=1.0):
    if loss_weights is None: loss_weights = {}
    # prep inputs
...
class MRF(Layer):
  ...
  def call(self, inputs):
    ...
    with tf.name_scope('inv_convariance'):
      ...
      inv_cov = tf.linalg.pinv(cov_reg + tf.random.uniform(tf.shape(cov_reg)) * 1e-8)
...
def categorical(y_logits, temp=1.0, sample=False, hard=True):
  ...
  if isinstance(sample, bool): y_soft = gumbel_softmax_sample(y_logits) if sample else tf.nn.softmax(y_logits,-1)
  else: y_soft = K.switch(sample, gumbel_softmax_sample(y_logits), tf.nn.softmax(y_logits,-1))
  if isinstance(hard,bool): y_hard = one_hot(y_soft) if hard else y_soft
  else: y_hard = K.switch(hard, one_hot(y_soft), y_soft)
